...
import {
  ...
  PAYMENTS, phoneDigits, round2, waLink,
} from '@/lib/store';
...
export default function Menu({ data }) {
  ...
  return (
    <>
      ...
      {sheet?.type === 'cart' ? (
        <CartSheet
          cart={cart} store={store} subtotal={subtotal}
...
function CartSheet({ cart, store, subtotal, onQty, onClose, onNext }) {
...
function CheckoutSheet({ cart, store, subtotal, open, onBack, onClose, onDone }) {
  ...
  const [f, setF] = useState(() => {
    ...
    return {
      ...
      name: saved.name || '', phone: saved.phone || '', street: saved.street || '', number: saved.number || '',
  ...
  return (
    ...
    >
      ...
      <div className="grid2">
        ...
        <div className="field"><label htmlFor="ck-phone">WhatsApp</label><input id="ck-phone" inputMode="tel" autoComplete="tel" placeholder="(86) 9 0000-0000" value={f.phone} onChange={set('phone')} /></div>
      </div>
      {f.mode === 'entrega' ? (
        <>
